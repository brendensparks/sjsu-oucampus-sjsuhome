...
(function($) {
	...
	$.fn.lightBox = function(settings) {
		...
		settings = jQuery.extend({
			...
			imageBtnPrev:			'assets/images/btn_prev.gif',			// (string) Path and the name of the prev button image
			imageBtnNext:			'assets/images/btn_next.gif',			// (string) Path and the name of the next button image
		...
		function _show_image_data() {
			...
			if ( settings.imageArray.length > 1 ) {
				$('#lightbox-image-details-currentNumber').html('On Campus ' + settings.txtImage + ' ' + ( settings.activeImage + 1 ) + ' ' + settings.txtOf + ' ' + settings.imageArray.length).show();
			}		
		}
		/**
		...
		function _set_navigation() {
			...
			$('#lightbox-nav-btnPrev,#lightbox-nav-btnNext').css({ 'background-image' : 'url(' + settings.imageBlank + ')' });
			...
			if ( settings.activeImage != 0 ) {
				if ( settings.fixedNavigation ) {
					...
					$('#lightbox-nav-btnPrev').unbind().hover(function() {
						$(this).css({ 'background-image' : 'url(' + settings.imageBtnPrev + ')' });
					},function() {
						$(this).css({ 'background-image' : 'transparent url(' + settings.imageBlank + ')' });
			...
			if ( settings.activeImage != ( settings.imageArray.length -1 ) ) {
				if ( settings.fixedNavigation ) {
					...
					$('#lightbox-nav-btnNext').unbind().hover(function() {
						$(this).css({ 'background-image' : 'url(' + settings.imageBtnNext + ')' });
					},function() {
						$(this).css({ 'background-image' : 'transparent url(' + settings.imageBlank + ')' });
					}).show().bind('click',function() {
						settings.activeImage = settings.activeImage + 1;
